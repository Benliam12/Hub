...
public class SettingManager
{
    ...
    private void DeleteFile(File f)
    {
        if(f.exists())
        {
            ...
            {
                f.delete();
                Hub.log.info("File : "+ f.getName() + " is now deleted !");
            }
            catch (Exception ex)
            {
                ex.printStackTrace();
        ...
        {
            Hub.log.info("Couldn't delete file : " + f.getName() + " (File doesn't exist !)");
        }
    }
    ...
    public void addConfig(String name,String directory)
    {
        if(!this.isConfig(name))
        {
            ...
            Hub.log.info("Added config with name : " + name);
        }
        else
        {
            Hub.log.info("Couldn't add config with name : " + name);
        }
    }
    ...
    private void removeConfig(String name){
        if(this.isConfig(name))
        {
            configs.remove(name);
            removeFile(name);
            Hub.log.info("Removed config : " + name);
        } else {
            Hub.log.info("Couldn't remove config with name : " + name);
        }
    }
    ...
    public void clear()
    {
        for(Map.Entry<String, FileConfiguration> config : this.configs.entrySet())
        {
            this.saveConfig(config.getKey(), config.getValue());
        }
        files.clear();
        configs.clear();
        Hub.log.info("Removed all Files & Configs");
    }
